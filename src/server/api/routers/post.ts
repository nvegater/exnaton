...
const mapMeasurementsToInsertParams = (measurements: RawMeasurement[]): InsertMeasurement[] => {
  return measurements.map((measurement) => {
    const meterAddress = measurement["0100011D00FF"] !== undefined ? "0100011D00FF" : "0100021D00FF";
    const valueAsString = measurement[meterAddress as keyof RawMeasurement];
    if (typeof valueAsString !== 'string') throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: 'Value is not a number' });
...
const fetchJsonDataAndParse = async (): Promise<RawMeasurement[]> => {
  ...
  rawDataArray.forEach(rawData => {
    ...
    if (!rawData || typeof rawData !== 'object' || !Array.isArray(rawData.data)) {
      throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: 'Invalid data structure in response' });
    }
...
export const measurementsRouter = createTRPCRouter({
  ...
  importData: publicProcedure.mutation(async ({ ctx }) => {
    ...
    if (measurement) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: 'Data already imported' });

    const rawMeasurements = await fetchJsonDataAndParse();
    const insertParams = mapMeasurementsToInsertParams(rawMeasurements);

    const insertedValues = await ctx.db.insert(energyMeasurements).values(insertParams).returning()
  ...
  getLatest: publicProcedure.query(({ ctx }) => {
    return ctx.db.query.energyMeasurements.findFirst({
      orderBy: (energyMeasurements, { desc }) => [
        desc(energyMeasurements.timestamp),
      ],
    });
